...
We tested whether the bacterial relative abundances we observed from FIT cartridges could be used to differentiate healthy patients from those with carcinomas using random forest models, as has been demonstrated with stool [@baxter2016microbiota]. Using DNA from the FIT cartridge, the optimal model utilized `r fit_canc_model$Kopt` OTUs and had an AUC of `r sprintf('%.3f', fit_canc_roc$auc)`. There was no significant difference in the AUC for this model and the model based on DNA isolated directly from stool, which used `r stool_canc_model$Kopt` OTUs and had an AUC of `r sprintf('%.3f',stool_canc_roc$auc)` (`r sprintf('p=%.2f', roc.test(fit_canc_roc, stool_canc_roc)$p.value)`). We also generated random forest models for differentiating healthy patients from those with any type of lesions (i.e. adenoma or carcinoma). There was no difference in AUC between the stool-based model with `r stool_les_model$Kopt` OTUs (AUC=`r sprintf('%.3f', stool_les_roc$auc)`) and the FIT cartridge-based model with `r fit_les_model$Kopt` OTUs (AUC=`r sprintf('%.3f', fit_les_roc$auc)`,`r sprintf('p=%.2f', roc.test(fit_les_roc,stool_les_roc)$p.value)`). These findings demonstrated that models based on bacterial DNA from FIT cartridges could be as predictive as DNA isolated directly from stool.
